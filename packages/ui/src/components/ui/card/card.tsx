...
const Card = ({ className, size = 'default', ...props }: CardProps) => (
  <div
    className={cn(
      'cn-card group/card flex flex-col gap-6 rounded-lg border bg-card py-6 text-card-foreground shadow-sm data-[size=sm]:gap-4 data-[size=sm]:py-4',
      className
    )}
    data-size={size}
    data-slot="card"
...
const CardTitle = ({ className, ...props }: ComponentProps<'div'>) => (
  <div className={cn('cn-card-title font-semibold leading-none', className)} data-slot="card-title" {...props} />
)
